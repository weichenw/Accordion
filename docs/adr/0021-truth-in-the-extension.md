...
at the next `context` hook) — this is what eliminates the one-turn lag. `agent_end` reconciles the
full array as a backstop for anything `message_end` missed (e.g. a message that finished with no
client attached).
