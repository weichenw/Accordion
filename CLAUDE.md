...
- `Truth.calibration`/`calTokens(n)` — a host-set, rev-stamped provider-anchored multiplier (`k = realTokens/estTokens`) DISPLAY consumers opt into; decision math (`canFold`/`protectedFromIndex`/`stats()`) stays on the raw estimate — see [ADR 0025](docs/adr/0025-token-calibration.md)
