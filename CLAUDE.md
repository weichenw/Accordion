...
- `Truth.calibration`/`calTokens(n)` — a host-set, rev-stamped provider-anchored multiplier (`k = realTokens/estTokens`). Stage 2: `protectedFromIndex()` sizes the protected tail against a calibrated threshold, `stats()` reports calibrated `liveTokens`/`fullTokens`, and every conductor decision read (`ViewBlock.tokens`/`foldedTokens`, `ConductorHost.countTokens`) is calibrated too — `budget`/`protectTokens`/`contextWindow` stay the literal, unconverted dial values (already meant in real tokens). `ViewBlock.rawTokens` is the deliberately narrow exception: an optional original wire estimate reserved for structural sign checks (for example, proving a DROP cannot insert more raw wire than it removes), never budget or pressure math. `canFold` itself carries no token threshold, so it inherits the calibrated boundary transitively through `protectedFromIndex`. See [ADR 0025](docs/adr/0025-token-calibration.md)
