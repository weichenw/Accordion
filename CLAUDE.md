...
- **authored digest** — a folded block's digest, or a folded group's summary, written by the HUMAN instead of the engine (`fold` op's `digest`, `group` op's `summary`; UI: `app/src/lib/ui/map/DigestEditor.svelte`). Stored verbatim in `Block.subst` / `Group.digest` and — the load-bearing part — **untagged**. See **the tag is the handle**. Clearing the editor means different things at the two granularities, deliberately: a BLOCK commits the `EMPTY_DIGEST` sentinel `{empty}` (~3 tok — per-block folding is content substitution, and `applyPlan` drops an empty `digestText`, which would ship the block WHOLE), while a GROUP commits a real DROP (`Group.digest === null`; a group may legally change the wire's message count, subject to the tool-pair fixpoint and role-validity floor). Saving with no text at all restores the engine's own digest/recap, tag included.
- **the tag is the handle** — the `{#<code> FOLDED}` prefix is the ONLY way a fold code ever reaches the model, so whether the wire text carries **that block's OWN tag** decides whether the agent can reach it at all (`core/digest.ts → hasOwnFoldTag`; enforced in `core/agentView.ts`'s `agentReachable`/`groupAgentReachable`). Own-tag, not any-tag: a foreign tag — pasted by a human, hand-authored by a conductor — must not make a block reachable by its own code, which is the accidental-restore path the rule exists to close. Engine digests, default group recaps, and a conductor's recoverable `replace` carry their own tag and are therefore `unfold`/`recall`-able; a HUMAN-authored digest and a conductor's `recoverable: false` replace are untagged and unreachable — a human's own words are not something the agent may undo. To keep that true, the HUMAN branches of BOTH `Truth.opFold` and `Truth.opGroup` **strip** leading tags (`stripFoldTags`, to a fixed point) exactly as `opReplace` does — the engine is the sole author of the tag. This is enforced in `Truth`, not in the editor, deliberately: the editor seeds its box with the current digest/recap, so edited engine text still starts with a tag, and `sanitizeOps` passes a raw `fold.digest`/`group.summary` through untouched — a widget-only strip would leave the invariant resting on a Svelte component. The strategy branches deliberately do not strip — `ViewConductor` emits fold-with-digest and a conductor authoring its own recall handle is a supported shape. The one carve-out is a DROP group: it emits no text, but stays reachable because the role-validity floor may degrade it to a `roleFloorRecap` stub carrying `foldTag(g.id)` on purpose.
...
| path | what |
|------|------|
| `core/` | Framework-free shared model: `Truth` (canonical context state, `truth.ts`), the v22 wire protocol (`protocol.ts`), replica (de)serialization (`replica.ts`), agent unfold/recall resolution (`agentView.ts`). Imported by the extension (relative `../core`) and the app (`$core` alias) — no Svelte, no Node-only deps |
...
| `conductors/ws/thermocline/` | Out-of-process conductor, dialing back over WebSocket: policy/epoch engine, Python attention probe, the spawned `runner.mjs`, and the committed `remote-sdk.mjs` bundle |
| `conductors/ws/triptych/` | Out-of-process conductor: pressure-gated thirds (raw tail / tree-sitter-L2 code-skeleton middle / lossy compaction-summary top), the spawned `runner.mjs`, the committed `triptych-sdk.mjs` bundle, and the wasm skeleton engine (`skeleton.mjs`, needs a one-time `npm install` there) |
...
| `app/src/lib/live/` | WS client (replica + remote control over protocol v22), session discovery, CC transcript browsing |
| `app/src/lib/ui/map/DigestEditor.svelte` | The folded digest, editable in place — always a real `<textarea>` (so select/copy/paste behave), with Save / Cancel / "Auto digest" and a live cost readout. `digestDrafts.ts` beside it holds unsaved text client-side, keyed by block/group id: a human unfold clears `subst`, and keeping the draft in `Truth` would mean a `SnapshotState` field and a protocol bump for what is really typing-in-progress |
...
- `core/types.ts` — `Block { id, kind, turn, order, text, tokens, toolName?, callId?, override, autoFolded, by, subst }`. Kinds: `system · user · text · thinking · tool_call · tool_result`. `system` is **bolted** (see Terminology) and always block 0 with id `SYSTEM_BLOCK_ID` (`sys:0`), `order` -1; it is created/replaced only by `Truth.setSystemPrompt` (host-only, from pi's `context` hook) and is emitted only when a prompt can actually be sourced. (`app/src/lib/engine/types.ts` re-exports the types; the two constants come from `$core/types`.)
...
- `Truth.calibration`/`calibrationThroughOrder` — a host-set, rev-stamped provider anchor (`k = realTokens/estTokens`) plus the last block order on the exact wire that receipt measured. Covered blocks use `calBlockTokens`; blocks appended afterward stay raw until a later receipt advances the frontier (issue #102). `protectedFromIndex()` and `stats()` use the same mixed covered/raw basis, and every conductor decision read (`ViewBlock.tokens`/`foldedTokens`, `ConductorHost.countTokens`) stays unit-consistent — `budget`/`protectTokens`/`contextWindow` remain literal real-token dials. `ViewBlock.rawTokens` is the narrow structural-sign exception. See [ADR 0025](docs/adr/0025-token-calibration.md)
...
| `conductors/ws/triptych/triptych-sdk.mjs` | bundled from `core/conductor/remote.ts` + `triptych.ts` (+ their `core/`/`conductors/in-process` graph) by the same `build-remote-sdk.mjs` | the SDK the triptych runner imports | **yes** — committed |

`remote-sdk.mjs` and `triptych-sdk.mjs` are the generated artifacts that ARE committed: `conductors/ws/` ships with the repo/desktop build, not with the npm package (the tarball's `files` list below has no `conductors/` entry), so the bundles have to already exist on disk rather than being produced by `npm run build`/`prepack`. Regenerate both by hand (one script emits them all) after touching `core/conductor/remote.ts`, `thermocline.ts`, `triptych.ts`, or their `core/` graph — see the Conductors section.
...
- `core/protocol.ts` — the v22 wire messages (`hello`/`snapshot`/`event`/`telemetry`/`commandResult`/`stream`/`conductorState`/`conductorStatus`/`wireDeparting`/`turnCommitted`/`proposeResult`/`completeResult`/`controller`/`notice` server→client; `command`/`resnapshot`/`propose`/`completeRequest`/`setConductorStatus`/`holdRelease`/`cancelComplete`/`claimController` client→server), `WireBlock`, `FoldOp`/`GroupOp`, `PROTOCOL_VERSION`, `DOOR_PORT`, and the `sanitizeCommand`/`sanitizeOps`/`sanitizeSurfaceId`/`sanitizeSurfaceLabel` ingress validators (v20 added the provider-receipt `calibrationThroughOrder` frontier so stale k never scales newly appended blocks — see [ADR 0025](docs/adr/0025-token-calibration.md); v21 added host-computed conductor readiness; v22 adds the bolted `system` block and removes the redundant `SnapshotState.systemPrompt` scalar while retaining its calibration-coverage bit)
...
- `core/groupShape.ts` — `messageKey` plus `collapsibleMessageKeys`/`hasCollapsibleCarrier`: the durability + tool-pair fixpoint deciding which of a group's MESSAGES the wire may actually remove. `Truth.classifyGroup` runs it (a group with no carrier is rejected as `"nothing collapses (all stragglers)"`), and a conductor calls the SAME function before proposing a group — `Truth.apply` validates group ops independently, so a doomed carrier proposed next to a sibling DROP silently loses content. A conductor must judge the set `opGroup` will ACTUALLY group, i.e. after `snappedRange` widens each boundary over its `messageKey`: trim every candidate run to a fixed point of that snap first (`safeRunFromUnits` in `conductors/ws/thermocline/policy.ts`, `snapToMessageAtoms` in `conductors/in-process/agedSummaryConductor.ts`) and propose exactly what was vetted
...
- **`triptych`** (`conductors/ws/triptych/`) — pressure-gated thirds (built from the skeleton-lab research on branch `claude/code-skeleton-extraction-rv0ovo`): inert until the visible window first crosses the 90% high-water mark, then divides the context into a raw bottom band (the most recent ~⅓ of the cap), a middle band where doorman-classified code reads become tagged, recoverable **tree-sitter L2 skeleton** `replace` folds (signatures kept, bodies elided; a decline-to-fold gate skips blocks a skeleton would not actually shrink), and a top band swept into one **lossy** summary group using compaction-naive's `COMPACTION_SYSTEM` verbatim (untagged, recursive — the agent has no path back; human detach recovers). Locks `human-steering`/`agent-unfold` (fully exclusive; `recall` stays available by contract). **Runs out of process** like thermocline: `runner.mjs` imports the committed `triptych-sdk.mjs` (same regenerate script as above) and injects the wasm skeleton engine (`skeleton.mjs` — needs a one-time `npm install` in `conductors/ws/triptych/`; the spawn fails loudly without it). `TriptychConductor` extends `AgedSummaryConductor` via three protected hooks (`agedBoundaryIndex`/`extraSavedTokens`/`promptTextOf`).
