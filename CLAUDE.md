...
- **the tag is the handle** — the `{#<code> FOLDED}` prefix is the ONLY way a fold code ever reaches the model, so its presence decides whether the agent can reach a block at all (`core/digest.ts → hasFoldTag`; enforced in `core/agentView.ts`'s `agentReachable`/`groupAgentReachable`). Engine digests, default group recaps, and a conductor's recoverable `replace` are tagged and therefore `unfold`/`recall`-able; a HUMAN-authored digest and a conductor's `recoverable: false` replace are untagged and unreachable — a human's own words are not something the agent may undo, and `foldCode` is a 6-char hash whose rare collisions would otherwise restore them by accident. The one carve-out is a DROP group: it emits no text, but stays reachable because the role-validity floor may degrade it to a `roleFloorRecap` stub that carries `foldTag(g.id)` on purpose.
